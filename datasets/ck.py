...
def ReadSegmentRGB(path, offsets, new_height, new_width, new_length, is_color, name_pattern):
    ...
    for offset_id in range(len(offsets)):
        ...
        for length_id in range(1, new_length+1):
            frame_name = path.split('/')[-1]+'_'+name_pattern % (length_id + offset)
...
def ReadSegmentFlow(path, offsets, new_height, new_width, new_length, is_color, name_pattern):
    ...
    for offset_id in range(len(offsets)):
        ...
        for length_id in range(1, new_length+1):
            ...
            frame_path_x = path + "/"+path.split('/')[-2]+'_' + frame_name_x
            cv_img_origin_x = cv2.imread(frame_path_x, cv_read_flag)
            frame_name_y = name_pattern % (length_id + offset)
            frame_path_y = path[:-6] + "y_flow"+ "/"+path.split('/')[-2]+'_' + frame_name_y
            cv_img_origin_y = cv2.imread(frame_path_y, cv_read_flag)

            # if cv_img_origin_x is None:
            #     print("Could not load file %s or %s" % (frame_path_x))
            #     sys.exit()
...
class ck(data.Dataset):
    ...
    def __init__(self,
        ...
        self.class_to_inew_lengthdx = class_to_idx
        self.clips = clips
        ...
        if name_pattern:
            ...
            if self.modality == "rgb":
                self.name_pattern = "%08d.png"
            elif self.modality == "flow":
                # self.name_pattern = "flow_%s_%08d.png"
                self.name_pattern = "%08d.png"
            ...
            for seg_id in range(self.num_segments):
                if self.phase == "train":
                    if average_duration >= self.new_length:
                        ...
                        offsets.apReadSegmentFlowpend(0)
                elif self.phase == "val":
                    if average_duration >= self.new_length:
